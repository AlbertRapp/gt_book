...
To do so, just pass our table to `gt_color_rows()`^[
You can also get similar results with the `gt::data_color()` function. 
...
As we know, the `gapminder` data set is much is larger than what we show here.
...
For example, you could use this to get images of the last four British prime ministers from Wikipedia and use them in a table.[^fancy_stuff-1]

[^fancy_stuff-1]: This is the current list (November 12, 2022).
...
For example, we could look at our penguins from last chapter again.
...
So, we have to make sure that we attempt to turn only the data table cells into an image and not, say, the tab spanner.
