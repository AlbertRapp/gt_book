...
Of course we will need data for that.
...
From that data let us compute a couple of simple counts.
To get started, this is probably an easy data set to put into a table.
...
In actual table, the data would probably be rearranged a bit.
...
![A not so sexy table of our `penguins_counts_wider` data set created by yours truly with LibreOffice Calc (spreadsheet software - double ugh. Though, compared to Excel it's open-source. So maybe 1.5 ugh?)](img/stupid_table_screenshot.png){#fig-terrible-tbl fig-align="center" width="90%"}
...
To change the column names use the `cols_layer()` "layer".
...
This joins columns and can be done with `tab_spanner()` layers.
...
Finally, while we're currently changing labels, let us add an important label - the title.
...
By the same trick we could also add a caption for a Quarto document (`tab_caption()`), a footnote (`tab_footnote()`) or another source note (`tab_sourcenote()`) here.
...
This assumes that your numbers use a font that assign equal width to all digits (monospace fonts).
